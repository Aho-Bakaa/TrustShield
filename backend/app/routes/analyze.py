"""Analysis endpoints: text/URL/social and audio upload."""
...
async def analyze_image(
    ...
    """Analyze an uploaded image (screenshot) OR a PDF.

    PDFs use embedded text when present; scanned/image PDFs and images go to the
    vision model. Either way the extracted text runs through the detection pipeline.
    """
    ext = Path(file.filename or "shot.png").suffix.lower()
    if ext not in _DOC_EXT:
        raise HTTPException(400, f"Unsupported file type '{ext}'. Use images {sorted(_IMAGE_EXT)} or .pdf")
    data = await file.read()
    ...
    result.evidence.insert(0, Evidence(
        source="vision", label=source_label,
