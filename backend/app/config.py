...
and the app selects a sensible default model for that provider. If no key is
present, the LLM layer transparently falls back to a rule-based mock so the
demo always runs.
"""
...
class Settings(BaseSettings):
    ...
    llm_model: str = ""  # if empty, derived from provider default
    ...
    render_screenshots: bool = True   # capture a screenshot for vision analysis
    ...
    triage_escalation_threshold: float = 0.25
    ...
    def vision_available(self) -> bool:
        """True when the vision model's provider has a key configured."""
        if self.force_mock_llm:
            return False
    ...
    def export_provider_key_to_env(self) -> None:
        """Ensure LiteLLM sees the key via the standard env var name."""
        key_env = _PROVIDER_KEY_ENV.get(self.llm_provider)
        explicit = getattr(self, f"{self.llm_provider}_api_key", "")
